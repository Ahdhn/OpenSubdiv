...
OsdOskiKernelDispatcher::ApplyM(int offset)
{
    ...
    if (A_tunable == NULL) {
        ...
        std::vector<int>* rowIndx =
            new std::vector<int>(M->index1_data().begin(), M->index1_data().end());
        std::vector<int>* colIndx =
            new std::vector<int>(M->index2_data().begin(), M->index2_data().end());
        std::vector<float>*  vals =
            new std::vector<float>(M->value_data().begin(),  M->value_data().end());
